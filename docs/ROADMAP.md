...
현재 v1(기본 아카이빙 앱)이 `main` 에 있습니다. 이 문서는 다음 작업의 설계안입니다.
**§7 의 추가 제안은 아직 확정이 아닙니다** — 승인해 주신 항목만 범위에 넣겠습니다.
...
1. **`options.yaml` 단일 파일 + UI 쓰기**가 맞는지.
   (대안: 저장소의 기본값 파일 + 사용자 오버레이 2단 구조 — 유연하지만 "내 설정이 어디 있지?"가 생깁니다.)
2. **주석 보존을 위해 `ruamel.yaml` 의존성**을 추가해도 되는지. (아니면 PyYAML, 대신 주석 소실)
3. **폰트 동봉 여부** — `assets/fonts/` 에 Pretendard + JetBrains Mono(각 OFL, 합계 약 3–6MB)를
   넣으면 어느 PC에서든 동일한 화면. 저장소를 가볍게 두려면 시스템 폰트 폴백만.
4. **라이트 테마도 유지**할지, 다크 전용으로 갈지. (토큰 구조상 유지는 쉽지만 QSS 검증 부담이 2배)
5. **§7 제안 중 채택할 번호.**
...
| Phase | 내용 | 산출물 |
|---|---|---|
| **P0** | DB 마이그레이션 프레임 + `config_store.py` + 테마 토큰 골격 | 기능 변화 없음, 기반만 |
| **P1** | 다크 테마 + 폰트 적용, 하드코딩 색 제거 | 눈에 바로 보이는 변화 |
| **P2** | `options.yaml` 연동 콤보박스 + 드롭다운 `＋` / 우클릭 / F2 | R1·R2·R3 |
| **P3** | Task별 컬럼 + 커스텀 필드(`extra_json`) + 헤더 우클릭 편집 | R4 |
| **P4** | GPU 인지형 서버 상태 패널 + GPU 선택 폼 | R5 |
| **P5** | 우클릭/F2 편집을 전 표면에 일관 적용 (`editing.py`) | R6 |
| **P6** | 승인된 §7 항목 | — |

각 Phase는 별도 커밋으로 쌓고, P1~P5 를 하나의 PR로 올리겠습니다.
