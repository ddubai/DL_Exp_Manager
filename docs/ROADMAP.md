...
콤보박스에서 **지표별 체크박스**로 바꿨다 - 기본은 파싱된 지표 전부 체크된
상태로 시작해서, 별도 조작 없이도 한 화면에 겹쳐 보인다. `CurveChartWidget`
도 `set_points`(단일 시리즈) 대신 `set_series`(다중 시리즈) 를 받도록 다시
짰다: 시리즈가 하나면 이전처럼 실제 값 축을 그대로 보여주고, 둘 이상이면
