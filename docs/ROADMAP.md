...
### 24.1 후속 수정 — 지표 이름의 방향 표시(↑/↓) 인식 + 여러 지표 동시 플롯

실제 `_loss_log.txt` 에는 `Average psnr(↑): 10.33` 처럼 지표 이름 뒤에 방향
표시(또는 단위)가 괄호로 붙어 있었다 - 기존 `_AVERAGE_KV_RE` 는 지표 이름 바로
뒤에 콜론이 온다고 가정해서 이런 줄은 통째로 못 잡았다. 콜론 앞에서 괄호/대괄호
덧말이나 화살표 같은 기호 하나를 건너뛰도록 정규식을 고쳤다(`(?:\([^)]*\)|
\[[^\]]*\]|[^\w\s:=/]+)?`) - `psnr(↑)`, `ssim(up)`, `PSNR (dB)` 모두 잡힌다.

동시에 "여러 지표를 같이 플롯"도 요청받아, `CurveDialog` 를 지표 하나만 고르는
콤보박스에서 **지표별 체크박스**로 바꿨다. `CurveChartWidget` 도
`set_points`(단일 시리즈) 대신 `set_series`(다중 시리즈) 를 받도록 다시 짰다:
시리즈가 하나면 이전처럼 실제 값 축을 그대로 보여주고, 둘 이상이면
loss/PSNR/SSIM 처럼 값의 범위가 서로 달라 같은 축을 못 쓰므로 시리즈마다
자기 자신의 최소/최대로 0~1 정규화해 겹쳐 그린다(축 라벨은 "high"/"low" 로
단순화). 체크박스 글자색을 그 지표 선 색(`theme.series_color`, 정렬된 키
순서로 고정 배정)과 맞춰서 별도 범례 없이 체크박스 자체가 범례 역할을
하게 했다 - 체크를 껐다 켜도 같은 지표는 항상 같은 색을 유지한다.

### 24.2 후속 수정 — 기본값을 "전부 켜짐"에서 "전부 꺼짐"으로

§24.1 은 지표 체크박스를 전부 켠 채로 시작했는데("Plot 을 같이"), 정작
"한 번에 다 겹쳐 그리지 말고 항목별로 켜고 꺼서 하나씩 확인하게 해 달라"는
요청을 받았다 - 지표가 여러 개면 처음부터 다 겹쳐 그려서 오히려 하나씩
살펴보기 어려웠다. `_rebuild_metric_checks` 의 기본 체크 상태를 꺼짐으로
바꿔서, 창을 열면 아무 것도 안 그려진 상태로 시작하고 사용자가 보고 싶은
지표만 하나씩(또는 여러 개를 동시에) 켠다. 아무 것도 안 켜졌을 때는
`CurveChartWidget` 이 "Not enough data points…" 대신 "Check a metric below
to plot it." 를 보여줘서, 데이터가 없는 것과 아직 아무 것도 안 고른 것을
구분한다. "All"/"None" 일괄 토글 버튼은 그대로 남겨 뒀다 - 여러 개를
한꺼번에 겹쳐 비교하고 싶을 때는 여전히 그렇게 할 수 있다.

### 24.3 후속 수정 — 전각 괄호·공백 섞인 지표 덧말이 그 지표만 조용히 빠뜨리던 버그

실사용 신고: "loss 는 켜면 잡히는데 psnr/ssim 은 개별로 켜도 안 잡힌다." 원인은
§24.1 에서 넣은 `_AVERAGE_KV_RE` 의 덧말 스킵 패턴이 `\([^)]*\)`(반각 ASCII 괄호)
/ `\[[^\]]*\]`(대괄호) / `[^\w\s:=/]+`(공백 없는 기호 나열) 세 가지만 허용했던
것 - 실제 로그의 `psnr(윗 화살표)` 는 한글 입력기 습관상 **전각 괄호(（）)** 로
쓰였거나, 반각 괄호라도 안에 **공백이 낀 두 단어**(`윗 화살표`)였다. 전각 괄호는
세 패턴 중 어디에도 안 걸렸고, 반각+공백 조합은 세 번째 "공백 없는 기호" 대체가
공백에서 끊겨 버려서, 그 지표 하나만 규칙에 안 걸리고 조용히 `_series` 에서
빠졌다 - 체크박스 목록에도 아예 안 나타나니 "개별로는 안 잡힌다"로 보였다.

괄호/화살표 종류를 하나하나 나열하는 대신, 지표 이름 뒤에서 **`/`, `:`, `=`,
줄바꿈이 아닌 문자는 뭐든(비탐욕적으로) 건너뛰고 콜론을 찾는** 방식으로 정규식을
단순화했다(`[^/:=\n]*?`) - 반각/전각 괄호, 공백 섞인 덧말, 화살표, 단위 표기를
