...
- **📊 Chart** 탭: 지표별 막대 그래프(지표마다 자기 스케일 안에서 정규화 - PSNR
  ...
  강조됩니다.
- **config.yaml Diff** 탭: 2개를 골랐으면 좌우로 나눈 diff - 추가된 줄은 초록,
  삭제된 줄은 빨강으로 표시되고 스크롤은 양쪽이 같이 움직입니다. 오른쪽 위
  **"Changed lines only"** 체크박스로 바뀐 줄만 보기 ↔ 전체 다 보기를 바꿀 수
  있습니다(기본은 바뀐 줄만). 3개 이상이면 Run 별 config 를 각각 탭으로.
